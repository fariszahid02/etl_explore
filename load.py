...
def load_latest_daily_donor(conn):
    ...
    if latest_url:
        ...
        r.raise_for_status()
        with open(os.path.join(FOLDER, "daily_donor.parquet"), "wb") as f:
            f.write(r.content)
        conn.execute("DROP TABLE IF EXISTS daily_donor")
        conn.execute(f"CREATE TABLE daily_donor AS SELECT * FROM read_parquet('{os.path.join(FOLDER, 'daily_donor.parquet')}')")
